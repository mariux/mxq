...
int main(int argc, char *argv[])
{
    ...
    for (i=0; i<group_cnt; i++) {
        ...
        printf("user=%s uid=%u group_id=%lu pri=%d jobs_total=%lu run_jobs=%lu run_slots=%lu failed=%lu finished=%lu inq=%lu job_threads=%u job_memory=%lu job_command=%s group_name=%s\n",
                ...
                g->job_threads, g->job_memory, g->job_command, g->group_name);
